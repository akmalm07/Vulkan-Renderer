...
BaseEngine::BaseEngine(tools::WindowT& glfwWindow, bool orthoOrPerpective, bool debug)
{
	...
	_window.CreateWindow(false, orthoOrPerpective);
...
BaseEngine::BaseEngine(GLFWwindow* glfwWindow, bool orthoOrPerpective, bool debug) 
{
	...
	_window = std::move(tools::WindowT(800, 800, "Vulkan"));
...
{ 
	_window = std::move(tools::WindowT(800, 800, "Untitled Window"));
	_window.CreateWindow(true, false);
}


void BaseEngine::build_glfw_window(int width, int height, bool orthoOrPerpective)
{ 
	_window = std::move(tools::WindowT(width, height, "Vulkan"));
	_window.CreateWindow(false , orthoOrPerpective);
}
...
void BaseEngine::make_pipeline()
{
	...
	spesifications.vertShader = UserInput::v_shader_path;
	spesifications.fragShader = UserInput::f_shader_path;
	...
	spesifications.vertShader = UserInput::line_v_shader_path;
	spesifications.fragShader = UserInput::line_f_shader_path;
	spesifications.topology = vk::PrimitiveTopology::eLineList;

	vkInit::GraphicsPipelineOutBundle lineOutput = vkInit::create_pipeline(spesifications, _debugMode);


	_vkLinePipeline = lineOutput.pipeline;
	_vkLinePipelineLayout = lineOutput.pipelineLayout;
	_vkLineRenderpass = lineOutput.renderpass;
...
void BaseEngine::record_draw_commands(vk::CommandBuffer& commandBuffer, uint32_t imageIndex)
{
	...
	commandBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics, _vkLinePipeline);
	commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, _vkLinePipelineLayout, 0, 1, _vkDescriptorSets.sets.data(), 0, nullptr);
...
void BaseEngine::load_line_buffer()
{
	...
	_lineBuffer = vkType::VertexBuffer(bufferInfo);

	std::array<float, 36> linesXYZ =
	{
		// X-axis (Red)
	...
	_lineBuffer.initalize(linesXYZ, _debugMode);

	DEBUG_ITERATOR(linesXYZ);
...
BaseEngine::~BaseEngine()
{
	...
	_vkLogicalDevice.destroyPipeline(_vkLinePipeline);

	_vkLogicalDevice.destroyPipelineLayout(_vkLinePipelineLayout);

	_vkLogicalDevice.destroyRenderPass(_vkLineRenderpass);
